...
def print_help():
    ...
    console.print(Panel.fit(
        "[bold magenta]FTP BruteForce - Purple Shiva Tools[/bold magenta]",
        subtitle="[bold yellow]Ferramenta de teste de força bruta FTP[/bold yellow]",
        style="magenta",
        box=ROUNDED
    ))
    ...
    param_table.add_row("0", "TARGET IP", "Endereço IP do servidor FTP", "[red]✓[/red]")
    ...
    examples.append("    python ftp_bruteforce.py -i 192.168.1.100 -u admin -p wordlist.txt\n\n")
    examples.append("  Com opções avançadas:\n", style="bold")
    examples.append("    python ftp_bruteforce.py -i 10.0.0.1 -u root -p passwords.txt \\\n")
    examples.append("      -d 0.5 -r xml -v\n\n")
    examples.append("  Modo silencioso (apenas relatório):\n", style="bold")
    examples.append("    python ftp_bruteforce.py -i 172.16.0.10 -u backup -p rockyou.txt > report.log")
    ...
    console.print(Panel(
        ...
        "[bold]Repositório:[/bold] github.com/purpleshiva/ftp-bruteforce",
        title="Sobre",
        border_style="green"
    ))
