...
def WritePdfLog(filePath, target1, target2, arpPacketsSent, capturedPackets, duration, protocolData=None):
    ...
    # --- Introduction ---
    introText = (
        "This report was generated by the Purple Shiva Tools ARP Spoofing module. "
        "It documents a man-in-the-middle attack between two targets, including "
        "network traffic analysis and security recommendations."
        "<br/><br/>"
    ...
    if allDomains:
        # Calculate rows per page (about 40 rows per page with current styling)
        rowsPerPage = 40
        ...
        for pageNum in range(totalPages):
            ...
            # Create table with all domains for current page
            elements.append(Table(
                ...
                style=TableStyle([
                    ...
                    ('FONTSIZE', (0,0), (-1,-1), 8),
                    ('VALIGN', (0,0), (-1,-1), 'TOP'),
                    ('WORDWRAP', (0,0), (-1,-1))  # Enable word wrap for long domains
                ])
            ))
            
            # Only add spacer if not last page
    ...
    if dnsStats.get("total", 0) > 0:
        topDnsQueries = protocolData["protocols"]["dns"]["queries"][:10]  # Show first 10 queries
        dnsQueriesData = [["Timestamp", "Source", "Domain", "Type"]]
    ...
    for recommendation in RECOMMENDATIONS:
        ...
        recommendationBlock += [
            Paragraph(f"• <b>{recommendation['title']}</b> (Severity: {recommendation['severity']})", bodyStyle),
            Spacer(1, 5),
            Paragraph(f"<i>{recommendation['description']}</i>", bodyStyle),
            Spacer(1, 5),
        ]

        # Add specific details if they exist
        details = recommendation.get('specificDetails', {})
        if details:
        ...
        for source in recommendation['sources']:
            recommendationBlock += [
                Paragraph(f"‣ {source}", bodyStyle),
                Spacer(1, 3),
            ]
        recommendationBlock.append(Spacer(1, 15))
        elements.append(KeepTogether(recommendationBlock))
