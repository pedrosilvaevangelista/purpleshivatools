    ...
    def _update_loop(self):
        while not self._stop_event.is_set():
            ...
            if self.total_tasks:
                ...
                output = f"Progress: {conf.BOLD}{percent:.2f}%{conf.RESET} | Duration: {conf.BOLD}{elapsed_formatted}{conf.RESET} | Ports: {completed}/{self.total_tasks}"
            else:
                output = f"Tested: {conf.BOLD}{completed} ports{conf.RESET} | Duration: {conf.BOLD}{elapsed_formatted}{conf.RESET}"
